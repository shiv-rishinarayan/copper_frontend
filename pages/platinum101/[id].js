  ...
  return (
    <div className="">
      ...
      <div className="flex flex-col mt-[3.5rem]">
        {/* header  */}
        <div className="bg-accent/10 px-3 md:px-20 py-16 mb-16">
          <Image
            src={card.image}
            alt={card.title}
            width={200}
          ...
          />
          <h1 className="text-[28px] font-bold mb-2 pl-[2px]">{card.title}</h1>
          <p className="text-[17px] text-gray-700 mb-4 pl-[2px]">{card.description}</p>
        </div>
        {/* Render detailed sections dynamically */}
        {card.details.map((section, index) => (
          <div key={index} className="mb-10 px-3 md:px-20">
